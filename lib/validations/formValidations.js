...
export const servicesSchema = yup.object().shape({
    services: yup.array().of(yup.string()).min(1, 'At least one service must be selected').required('service is required'),
    message: yup.string().required('message is required'),
    images: yup.array().max(3, "You can't upload more than 3 images"),
});