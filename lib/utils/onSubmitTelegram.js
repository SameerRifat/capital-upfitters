export const submitFormToTelegram = async (data) => {
    const TELEGRAM_BOT_TOKEN = "6652666730:AAGEJi2jbmttBxkrc-f5jItmu4XJhqCQhm4";
    const TELEGRAM_CHAT_ID = "-1002128482422";
    try {
        const message = `
        Contact Information:
            - Name: ${data.contactDetails.firstName}
            - Last Name: ${data.contactDetails.lastName}
            - Phone: ${data.contactDetails.phoneNumber}
            - Email: ${data.contactDetails.email}

        Car Information:
            - Year: ${data.vehicleInformation.year}
            - Make: ${data.vehicleInformation.make}
            - Model: ${data.vehicleInformation.model}
            - License Plate: ${data.vehicleInformation.licensePlate}
            - State: ${data.vehicleInformation.state}

        Services: ${data.servicesInformation.services.map((service) => `- ${service}`).join('\n')}

        Comments: (${data.servicesInformation.message})
        `;

        if (data.servicesInformation.images && data.servicesInformation.images.length > 0) {
            const photoArray = Array.from(data.servicesInformation.images);

            const mediaGroup = photoArray.map((_, index) => ({
                type: 'photo',
                media: `attach://photo_${index}`,
                caption: index === 0 ? message : undefined,
            }));

            const formData = new FormData();
            formData.append('chat_id', TELEGRAM_CHAT_ID);
            formData.append('media', JSON.stringify(mediaGroup));

            photoArray.forEach((photo, index) => {
                // Convert data URI to Blob
                const byteString = atob(photo.split(',')[1]);
                const mimeString = photo.split(',')[0].split(':')[1].split(';')[0];
                const ab = new ArrayBuffer(byteString.length);
                const ia = new Uint8Array(ab);
                for (let i = 0; i < byteString.length; i++) {
                    ia[i] = byteString.charCodeAt(i);
                }
                const blob = new Blob([ab], { type: mimeString });
        
                formData.append(`photo_${index}`, blob, `photo_${index}`);
            });

            const photoResponse = await fetch(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMediaGroup`, {
                method: 'POST',
                body: formData,
            });

            if (!photoResponse.ok) {
                console.error('Error sending photos to Telegram:', photoResponse.statusText);
            } else {
                return { success: true };
            }
        } else {
        const textResponse = await fetch(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage`, {
            method: 'POST',
            body: new URLSearchParams({
                'chat_id': TELEGRAM_CHAT_ID,
                'text': message,
                // 'parse_mode': 'html',
                'parse_mode': 'Markdown',
            }),
        });

        if (!textResponse.ok) {
            console.error('Error sending text to Telegram:', textResponse.statusText);
        } else {
            return { success: true };
        }
        }

        return { success: false, error: 'Unknown error' };
    } catch (error) {
        console.error('Error sending message to Telegram:', error);
        return { success: false, error: 'Unknown error' };
    }
};
